...
<style>
  ...
  /* ── THE HOUSE'S LINES ────────────────────────────────────────────
     ...
     press it and the row walks to that shelf. The one in view reads
     at full strength and the rest at half, as opacity rather than a
     colour, so it holds on the cover's dark ground and the bar's paper
     alike. */
  .fam .famsep, .fam .famgo { opacity: 0.5; }
  .fam .famgo { cursor: pointer; }
  .fam .famgo.on { opacity: 1; }
  @media (hover: hover) { .fam .famgo:hover { opacity: 1; } }
