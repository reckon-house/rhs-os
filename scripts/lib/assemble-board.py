...
/* ── AND NEVER BACKWARDS ─────────────────────────────────────────
   ...
   the direction of travel — a column gone past to the left is gone,
   and the one after it settles in — with a hand's width of tolerance
   at the edge itself, so a stop right on a column is that column.
   Only a movement too small to be meant (24px) goes back where it
   started. */
const landX = () => {
  flowFrom = null;
  const x = (tgt.x + GAP / 2) / MOD_X;
  const i = Math.abs(flowRun) <= 24 ? Math.round(x)
    : flowRun > 0 ? Math.ceil(x - 0.04) : Math.floor(x + 0.04);
  pageTo(i);
};
