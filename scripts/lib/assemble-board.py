...
/* ── ONE PATH, EVERY RUNG ───────────────────────────────────────────
   ...
   it actually wrote. The browser then decodes the file that fits the
   box rather than the biggest one there is, which is the whole point:
   a 96px row in a list was holding a 1536px bitmap. */
const srcsetFor = (t) => {
  if (!t.r || !t.r.length) return null;
...
function studyRows(list, into, from) {
  ...
  list.forEach(({ folder, g }) => {
    ...
    const im = el("img"); im.src = cv ? encodeURI(cv.t) : ""; im.alt = ""; im.loading = "lazy";
    /* the row's picture is 96px wide and was holding a 1536px bitmap */
    if (cv) { const ss = srcsetFor(cv); if (ss) { im.srcset = ss; im.sizes = "96px"; } }
    const t = el("div");
