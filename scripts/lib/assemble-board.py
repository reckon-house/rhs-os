...
<style>
  ...
  #coverline { position: fixed; z-index: 21;
...
/* ── THE FIELD OPENS ON HOVER ───────────────────────────────────────
   ...
   folds back to the last thing said. A beat before folding, so it
   never flickers while being read, and never while the cursor is in
   the input. Pointer only; a phone taps. */
if (!PHONE && matchMedia("(hover: hover)").matches && askEl) {
  let foldT = 0;
  const unfold = () => { clearTimeout(foldT); openThread(); };
  const fold = () => {
    clearTimeout(foldT);
    foldT = setTimeout(() => {
  ...
  askEl.addEventListener("pointerenter", unfold);
  askEl.addEventListener("pointerleave", fold);
  thread.addEventListener("pointerenter", () => clearTimeout(foldT));
  thread.addEventListener("pointerleave", fold);
  askInput.addEventListener("blur", fold);
}
