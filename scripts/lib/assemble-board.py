...
/* ── A PHONE IS DEALT LIKE THE DESKTOP, IN PAIRS ────────────────────
   ...
   STACKED WITHIN THE PAIR, not fitted to rows. The statement is 330px
   tall in a 158px column, and a row fitted to it left a hole that
   deep under whatever stood beside it; the desktop's statement is
   half that in a column twice as wide, which is why fitted rows cost
   it nothing. Each column runs its own height here, as the phone did
   before, and the air is a feed's, not a board's. */
const BOARD = deal(items, PHONE
  ? { lead: { kind: "statement", w: COL, h: STATEMENT_H }, stack: true, air: [24, 64] }
...
const pageTo = (i) => {
  /* a phone turns whole pages: two modules, the pair. It used to stop
     at the last open column, because the field was one period and
     nothing stood past it; the field runs on now, as the desktop's
     does, and so does the row. */
  if (PHONE) i = Math.round(i / PAGE) * PAGE;
...
function insertColumn(c, from, opts) {
  ...
  if (from && ccols.indexOf(from) >= 0) {
    ...
    /* on a phone a column spans both modules of a pair, so it stands
       after the PAIR that asked and never inside it: anchored between
       a pair's two columns it would split the grid. u | 1 is the right
       column of whichever pair u is in, in any period. */
    if (PHONE) u = u | 1;
    c.__after = u;
