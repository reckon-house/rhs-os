...
<style>
  ...
  /* ── AND DOWN, ON THE LINE ─────────────────────────────────────────
     ...
     nub on its rule. */
  #rules i .yc { position: absolute; left: 50%; top: calc(100% + var(--count-gap, 8px)); transform: translateX(-50%);
  ...
  #rules i .yc:empty { display: none; }
  #rules i .yc > span { display: block; }
...
function dressRules() {
  ...
  const picsLeft = (f) => {
    if (!f || f.__c == null || f.__lines) return null;
    const max = f.scrollHeight - f.clientHeight;
    if (max <= 1) return null;
    const pics = (byCol[f.__c] || []).filter((t) => t.kind === "img").sort((a, b) => a.y - b.y);
    if (pics.length < 2) return null;
    const p = Math.min(1, Math.max(0, f.scrollTop / max));
    const line = f.scrollTop + p * f.clientHeight;
    let crossed = 0;
    for (const t of pics) if (t.y <= line) crossed += 1;
    return pics.length - Math.max(0, crossed - 1);
  };
  ...
  for (let k = 0; k < kids.length; k++) {
    ...
    const n = atDisp(d).col ? null : picsLeft(scrollerAt(d - 1));
    const key = n == null ? "" : String(n);
    if (yc.__n === key) continue;
    yc.__n = key;
