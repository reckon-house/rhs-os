...
function deal(list, opts) {
  ...
  const colH = new Array(cols).fill(ys[rows]);
  const out = [], cc = {};
  ...
  sized.forEach((it, i) => {
    ...
    let y = ys[k];
    if (colY) { y = colY[c]; colY[c] += h + airOf(); colH[c] = y + h; }
  ...
  return { tiles: out, ROWS: rows, COLS: cols, PW: cols * MOD_X,
    PH: colY ? Math.max(...colH) : ys[rows], colH, rowY: ys, coverCol: cc };
}
function adopt(L) {
...
const BOARD = deal(items, PHONE
  ? { lead: { kind: "statement", w: COL, h: STATEMENT_H },
      ...
      air: [24, 64] }
  : { lead: { kind: "statement", w: COL, h: STATEMENT_H }, stack: true });
adopt(BOARD);
