...
<style>
  ...
  /* ── THE PREVIEW: a study, down the column it was tapped in ───────
     ...
     every picture the study holds, at column width, scrollable, with
     the way to the study itself at its head. The column it opens in
     is the column the tile was in: the field pages there first, so
     the preview always arrives in the slot the eye was already on. */
  #preview {
  ...
  #preview[data-on] { grid-template-rows: 1fr; pointer-events: auto; }
  #preview > div { overflow: hidden; min-height: 0; }
  #previewIn {
    height: calc(100dvh - var(--nav));
    background: var(--paper, #fff);
    padding: 26px var(--thread-pad, 20px) 40px;
    overflow-y: auto; -webkit-overflow-scrolling: touch;
  }
  #previewHead { margin-bottom: 22px; }
  #previewHead .pvt {
    font-size: clamp(20px, 2.2vw, 30px); font-weight: 600;
    line-height: 1.22; letter-spacing: -0.04em;
  }
  ...
  #previewIn figure { margin: 0 0 18px; }
  #previewIn img { display: block; width: 100%; height: auto;
    border-radius: 14px; background: rgba(0, 0, 0, 0.04); }
  #previewCount { font-size: 11px; letter-spacing: 0.04em;
    color: rgba(0, 0, 0, 0.35); margin: 4px 0 18px; }
...
/* ── THE ROWS ARE DEALT, THE CONTENT FITS THE ROWS ─────────────────
   Row height = max(content) was the trap: one tall portrait anywhere
   in a row's fifty columns inflated that row for everyone, and the
   front page lost its grid to a stranger three screens to the right.
   So the heights are DEALT — tiers off the seed, shared by every
   column, the same move the phone grid makes with its shapes — and a
   tile taller than its row renders SMALLER rather than pushing
   fifty columns down. An image scaled to its row is honest thumbnail
   behavior; a row scaled to one image was weather.

   Row zero is the statement's own: nothing in the first row stands
   taller than the intro, so the field opens level. Quotes are text
   and cannot scale — a long one spends its row's air instead, which
   the air can afford. */
...
for (let r = 0; r < ROWS; r += 1) {
  if (r === 0) { rowH[0] = STATEMENT_H; continue; }
  let t, guard = 0;
  do { t = ROW_TIERS[Math.floor(rnd() * ROW_TIERS.length)]; }
...
sized.forEach((it, i) => {
  ...
  let { w, h } = it;
  if (it.kind === "img") {
    const budget = rowH[r] - (GROUPS[it.g] ? CAP_H : 0);
    if (h - (GROUPS[it.g] ? CAP_H : 0) > budget) {
      const imgH = h - (GROUPS[it.g] ? CAP_H : 0);
      const f = budget / imgH;
      w = Math.max(60, Math.round(w * f));
      h = budget + (GROUPS[it.g] ? CAP_H : 0);
    }
  }
...
field.addEventListener("wheel", (e) => {
  e.preventDefault();
  if (threadOpen || previewOpen) return;
...
/* ── THE PREVIEW ────────────────────────────────────────────────────
   Every picture the study holds, down the column the tile was tapped
   in — the field pages to that column first, so the preview always
   arrives in the slot the eye was already on. Full size and
...
function openPreview(tile) {
  ...
  document.getElementById("previewCount").textContent =
    shots.length + (shots.length === 1 ? " picture" : " pictures");
  const box = document.getElementById("previewImgs");
  box.innerHTML = "";
  shots.forEach((i) => {
    const fig = document.createElement("figure");
    const im = document.createElement("img");
    im.src = encodeURI(i.t);
    im.alt = "";
    im.loading = "lazy";
    im.decoding = "async";
    fig.appendChild(im);
