...
<style>
  ...
  /* ── A CREDIT IS A ROW, AND ITS MARK IS THE LINE ──────────────────
     ...
     stays as it is. */
  /* the name holds the row's left edge like every other head, and the
     mark answers it at the right — the two ends of one line, which is
     what a credit is: who, and their own hand. */
  /* HALF THE ROW, NOT `auto`. An auto track sizes to the widest mark
     and every other one then sits at its own width inside it, so the
     right edges came out eleven different numbers — and a max-width in
     per cent resolved against that same track, which squeezed each
     mark against its neighbours. A fixed share ends the circle: every
     box is the same box, `contain` fits each mark inside it at its own
     ratio, and they all end on one line. */
  .ccol .crow.mark { grid-template-columns: minmax(0, 1fr) 50%;
    column-gap: 20px; align-items: center; padding-bottom: 40px; }
...
const MARK_H = { "Nordstrom": 16, "Rejuvenation": 17, "Visual Comfort": 15,
  "Floor & Decor": 19, "Design Within Reach": 22 };
...
async function openHouseColumn(kind, from, opts) {
  ...
  if (isInfo) {
    ...
    h.credits.forEach((name) => {
      const r = textRow(c.__matter, name, "");
...
const NEWS = [
  ...
  ["Sally OS is rolling out to the enterprise",
    "Eight apps, an AI strategist with 21 tools, an asset hub and a store-associate site, built from inside the marketing team in about four months. It thinks on its own now: live signals in, proposed campaigns out.",
