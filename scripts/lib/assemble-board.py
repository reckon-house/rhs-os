...
<style>
  ...
  #plane .tile.ixrow { position: absolute; display: block;
    ...
    transition: transform 0.62s cubic-bezier(0.16, 1, 0.3, 1),
      translate 0.5s cubic-bezier(0.2, 0.55, 0.2, 1); }
  /* ...except while a page turn is scrubbing it, where the transform
     ...
     step aside keeps its clock through a turn, since a column can
     open and the row glide to it in the same moment */
  #plane.turning .tile.ixrow { transition: translate 0.5s cubic-bezier(0.2, 0.55, 0.2, 1); }
...
function remount() {
  ...
  for (let i = Math.floor(x0 / PW); i * PW < x1; i++) {
    ...
    for (let j = 0; j < 1; j++) {
      for (let k = 0; k < tiles.length; k++) {
        ...
        /* the tile's module as dealt, by column and period, not by x,
           since a narrow frame sits in from its column's edge and its
           x alone would lie; and how far it stands aside for the
           columns anchored before it. The period before the statement
           sits off the left edge, seen only on an overdrag, and no
           column is anchored before it, so it never moves. Tested
           where it is GOING: the translate's transition carries it. */
        const u = i * COLS + t.col;
        const sh = shiftAt(u);
        if (gx + sh + t.w < x0 || gx + sh > x1 || gy + t.h < y0 || gy > y1) continue;
        const key = k + ":" + i + ":" + j;
        want.add(key);
        if (!live.has(key)) {
          const el = mount(t, gx, gy, u, sh);
          live.set(key, el);
  ...
  for (const [key, el] of live) {
    ...
    /* a tile still sliding aside for a column is tested where it is
       going, and can be out of the window there while still on the
       screen here: it stays until it has actually left */
    const r = el.getBoundingClientRect();
    if (r.right > -MARGIN_X && r.left < innerWidth + MARGIN_X &&
        r.bottom > -MARGIN_Y && r.top < innerHeight + MARGIN_Y) continue;
...
plane.addEventListener("click", (e) => {
  ...
  if (pv) {
    ...
    openStudyColumn(tile.dataset.slug, { preview: true, at: tile.__u }); return;
  }
  ...
  if (shot) { const tile = shot.closest(".tile"); openStudyColumn(tile.dataset.slug, { preview: true, at: tile.__u }); }
});
...
function colNode(kind, caption) {
  ...
  c.__line = line; c.__head = head; c.__body = body; c.__caption = caption; c.__kind = kind;
  ...
  line.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      /* an empty head takes the offer standing in it */
      if (t) askFrom(c, t); else if (c.__next) openStudyColumn(c.__next, {}, c);
    }
    if (e.key === "Escape") { line.value = ""; narrowColumn(c, ""); line.blur(); }
  });
  /* and a click on the offer is the same act, without the keyboard */
  line.addEventListener("click", () => {
    if (c.__next && !line.value) openStudyColumn(c.__next, {}, c);
  });
...
function openStudyColumn(folder, opts, from) {
  ...
  /* ── THE HEAD OF A STUDY IS THE WAY ONWARD ──────────────────────
     It used to repeat the study's own title, which is written again
     two lines below it, so the one line at the top of the column said
     nothing new. A study is also the only column that is a dead end:
     a list and an answer both hold rows to click, and a study holds
     only itself. So the head names the study the house would go to
     next and opens it with a click — the field is still a field, and
     typing over the offer asks instead. */
  const nx = predictNext(folder);
  const ng = nx && GROUPS[nx.slug];
  const c = colNode("study", ng ? "Next \u00b7 " + ng.t : g.t);
  c.__folder = folder;
  /* the path names what a column IS, not what it offers: the head
     says "Next · Hill Country home" and the rail must still say
     "A.R.C.", or the trail behind you reads as a row of predictions */
  c.__label = g.t;
  if (ng) {
    c.__next = nx.slug;
  ...
  /* ── A CASE ARRIVES WITH A PICTURE ────────────────────────────────
     A column of nothing but type is a column nobody believes is about
     photographs. The study's own cover stands from the moment it
     opens — the same frame the homepage leads it with — and Preview
     loads the rest UNDER it rather than being the only way to see
     anything at all. Closing the preview takes the rest away and
     leaves the cover. */
  const pics = el("div", "cpics"); c.__body.appendChild(pics);
  const files = (window.BOARD_ITEMS || []).filter((i) => i.g === folder);
  const firstSrc = coverOf[folder] || (files[0] && files[0].t);
  const put = (src, i) => {
    const im = el("img"); im.src = encodeURI(src); im.alt = ""; im.loading = "lazy";
    im.style.animationDelay = Math.min(0.4, i * 0.05) + "s";
    pics.appendChild(im); return im;
  };
  if (firstSrc) put(firstSrc, 0);
  const rest = files.filter((f) => f.t !== firstSrc);
  const load = () => {
    if (pics.childElementCount > 1) {
      [...pics.children].slice(1).forEach((n) => n.remove());
      pv.textContent = "Preview"; return;
    }
    rest.forEach((f, i) => put(f.t, i));
    pv.textContent = rest.length ? "Close the preview" : "Preview";
  };
...
window.__b = { cur, tgt, START, pageTo, placeAsk, askFrom, openStudyColumn, closeColumn, closeAllColumns,
  showCol, get ccols() { return ccols; }, dispOf, atDisp, shiftAt, VISIBLE, get live() { return live; }, get colIdx() { return colIdx; },
