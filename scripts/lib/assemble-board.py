...
<style>
  ...
  /* ── THE HOUSE HOLDS THE LEFT ─────────────────────────────────────
     New things go rightward, always: the intro anchors the far left
     and every column opens to the right of the last, deeper and
     deeper, the way a column browser has worked since the NeXT. Two
     modules of the strip are visible at once, so while a conversation
     is open the field narrows to ONE — its intro column — and the
     newest column stands beside it. Close everything and it opens
     back to its full width. */
  #field {
    position: relative; flex: none; height: 100%;
    overflow: hidden; touch-action: pan-y;
    /* the BASIS is what a flex item is actually laid out from, and
       `flex: none` leaves it `auto`: an animated width on this item
       was being ignored in favour of the basis the first layout
       resolved, so the field simply refused to narrow. Both are set
       together, and both transition. */
    transition: width 0.5s cubic-bezier(0.2, 0.55, 0.2, 1),
      flex-basis 0.5s cubic-bezier(0.2, 0.55, 0.2, 1);
  }
...
};
{
  ...
  document.addEventListener("click", (e) => {
    ...
    if (term && window.askFrom) askFrom(null, term.dataset.ask);
  });
}
...
/* ── ONE MODULE OF CONTEXT TO THE LEFT ──────────────────────────────
   A column is shown with whatever stands before it — the intro for
   the first, the column it was asked from for the rest — so the path
   reads as a path and never as one card floating alone.

   ARITHMETIC, NOT MEASUREMENT. offsetLeft was read while the field
   was still mid-narrow, so the strip aimed at where the column had
   been half a second earlier and scrolled clean past the intro. With
   the conversation open the field is one module and every column is
   one module, so the nth column's left edge is (n+1) modules and its
   context begins at n. */
const stripShow = (c) => {
  const i = c ? ccols.indexOf(c) : -1;
  stripTgt = i < 0 ? 0 : i * MOD_X;
};
/* the field's width IS the strip's grammar: full while it is alone,
   one module while the conversation runs beside it */
function fitField() {
  const w = (ccols.length ? MOD_X : FIELD_W) + "px";
  field.style.width = w; field.style.flexBasis = w;
}
...
function closeAllColumns() {
  ...
  applyFromColumns(); fitField(); stripShow(null);
}
function stripStep(dir) {
  /* the stops, left to right: the field, then every column, each one
     module apart — the same arithmetic stripShow uses */
  const here = Math.round(stripTgt / MOD_X);
  const n = Math.max(0, Math.min(ccols.length - 1, here + dir));
  stripShow(ccols.length ? ccols[n] : null);
}
