...
function remount() {
  ...
  if (fresh.length) {
    ...
    /* On a first load the reveal edge IS the arrival, so a tile has
       to be standing there, opaque, before the edge reaches it: the
       class is handed over with its fade switched off, and the fade
       comes back once the field is built. Everything after the first
       screen arrives the way it always did. */
    fresh.forEach((c) => {
      if (!booting) { arrive.observe(c); return; }
