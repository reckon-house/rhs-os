...
<style>
  ...
  /* ── THE CREDITS ARE A WALL ───────────────────────────────────────
     Two across, hairlines between, every mark hanging from its cell's
     left edge the way a caption hangs from its picture. Still. They
     were a reel first, cut through with the cover reel's own burn,
     and two logos of different widths on black both showed through
     it — the mechanism was right for eight photographs that fill a
     box and wrong for marks that do not. Six ways were tried
     (credits-variants.html); Jeremy took the grid. A mark is drawn to
     fit, not to fill, and reversed to white in the black room; the
     one that is a photograph of a mark stays as it is. A credit with
     no mark stands as its name, in the furniture register. */
  .ccol .cgrid { display: grid; grid-template-columns: 1fr 1fr; margin-top: 0.9em;
    border-top: 1px solid rgba(255, 255, 255, 0.28); }
  .ccol .ccell { height: 84px; padding: 20px 22px; border-bottom: 1px solid rgba(255, 255, 255, 0.28); }
  .ccol .ccell:nth-child(odd) { border-right: 1px solid rgba(255, 255, 255, 0.28); }
  .ccol .cmk { display: block; width: 100%; height: 100%; object-fit: contain; object-position: left center; }
  .ccol .cmk:not([src$="dwr.jpg"]) { filter: invert(1); }
  .ccol .cword { display: flex; align-items: center; height: 100%; font-size: var(--note);
    font-weight: 500; letter-spacing: 0.04em; text-transform: uppercase; color: rgba(255, 255, 255, 0.7); }
...
async function openHouseColumn(kind, from, opts) {
  ...
  if (isInfo) {
    ...
    h.credits.forEach((name) => {
      ...
      } else cell.appendChild(el("span", "cword", name));
      grid.appendChild(cell);
    });
    c.__matter.appendChild(grid);
  } else {
