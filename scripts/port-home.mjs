 ...
 *
 *   node scripts/port-home.mjs        rewrite the three generated files
...
const CURTAIN_OUT = "src/components/shell/pressing-curtain.css";
...
/* ── 3b. the curtain leaves with a stylesheet of its own ──
   The page transition is shell furniture: PressingTransition draws it
   on every route, and the head script draws its arrival cover on every
   route. Its rules sat in the homepage's stylesheet, so a curtain was
   only styled on a page that happened to load that sheet. When the old
   footer stopped carrying the sheet onto the daybook and the case
   studies, their curtains drew black words on a black panel, never
   lapped, and blinked away instead of lifting.

   So the block moves, whole, from its opening note to the ring's, and
   takes the three tokens it reads with it, set on the curtain itself so
   no page has to define them. Anchored on both notes and asserted: a
   rename in the lab fails the port instead of shipping a bare curtain. */
const CURTAIN_START = "/* ── the page transition";
const CURTAIN_END = "/* ── the ring: the index as the body";
const cStart = cssOut.indexOf(CURTAIN_START);
const cEnd = cStart === -1 ? -1 : cssOut.indexOf(CURTAIN_END, cStart);
if (cStart === -1 || cEnd === -1) throw new Error("port: the curtain block's notes were not found");
const curtainBlock = cssOut.slice(cStart, cEnd).trimEnd() + "\n";
cssOut = cssOut.slice(0, cStart) + cssOut.slice(cEnd);
const labToken = (rx, what) => {
  const m = css.match(rx);
  if (!m) throw new Error("port: curtain token not found in the lab: " + what);
  return m[1].trim();
};
const PAPER = labToken(/--paper:\s*([^;]+);/, "--paper");
const INK = labToken(/--ink:\s*([^;]+);/, "--ink");
const GUT = labToken(/--gut:\s*([^;]+);/, "--gut");
const GUT_PHONE = labToken(/:root\s*\{\s*--nav:\s*[^;]+;\s*--gut:\s*([^;]+);\s*\}/, "the phone --gut");
...
/* ── 4. the facts arrive by dynamic import, not by fetch ──
   TRANSPORT ONLY. The lab fetches a file beside itself because a page
   opened from disk has no bundler; here the same request goes through
   import(), so Turbopack splits the index into its own chunk and the
   URL survives a content hash.

   It used to be a STATIC import assigned straight to a const, which
   read as tidier and put 195.8KB brotli — 47% of the homepage's
   JavaScript — into the first load of a page most visitors never ask a
   question on. The laziness now lives in the lab, where it can be seen
   working; this only changes how the bytes arrive. */
const FETCH_RX =
  /factsWanted = fetch\("project-facts\.min\.json"\)\n\s*\.then\(\(r\) => \(r\.ok \? r\.json\(\) : null\)\)/;
if (!FETCH_RX.test(jsOut)) throw new Error("port: facts fetch call not found");
jsOut = jsOut.replace(FETCH_RX,
`factsWanted = import("@/data/generated/project-facts.min.json")
    .then((m) => m.default)`);
/* ── 4b. the homepage tiles get a srcset ──
   The lab cannot: opened from disk there is no optimizer behind it, so
   it declares a stub that returns "" and the call site skips the
   attribute. Here the same call reaches the helper every plate on the
   site already uses, so the tiles and the case studies ask for their
   sizes the same way and there is one ladder to change. */
const TILE_RX = /\/\* Swapped at port time\. See the note at the call site\. \*\/\nfunction tileSrcSet\(\) \{ return ""; \}/;
if (!TILE_RX.test(jsOut)) throw new Error("port: tileSrcSet stub not found");
jsOut = jsOut.replace(TILE_RX,
`function tileSrcSet(src) { return plateSrcSet(src) || ""; }`);

/* ── 4c. the work list comes from projects.ts, not from the lab ──
   ONE LIST. The lab declares its own thirty-row WORK because a page
   opened from disk has no bundler and no import; the app has both, and
   keeping a second copy is what let the two surfaces disagree — a tile
   pinned to 0.86 on the homepage and dealt 0.33 in the ring, because
   only the lab's copy carried the authored size.

   The tuple shape is the lab's, unchanged, so nothing downstream of
   WORK has to know where the rows came from:
     [title, category, href, image, width, height, keywords, size, drift]

   Dimensions come from image-dimensions.ts, which is the same table
   the rest of the site measures against, so a recropped tile updates
   in one place. A tile the registry does not know still gets a pair
   rather than undefined — scripts/check-dimensions.mjs fails the build
   before that can happen, and this fallback only exists so a missing
   entry degrades to a square instead of throwing mid-deal. */
const WORK_RX = /const WORK = \[[\s\S]*?\n\];/;
if (!WORK_RX.test(jsOut)) throw new Error("port: lab WORK literal not found");
jsOut = jsOut.replace(WORK_RX,
...
/* ── 4d. the index hover comes from lib, not from the lab ──
   ONE IMPLEMENTATION. armRows writes --ix-grow and the --drop that
   makes room for it, and the ring in the footer never ran it: every
   frame down there took the CSS fallback of 1.32 with no drop, so it
   inflated a third while the page made no room and covered its own
   caption. The ring could not call it because everything in this file
   is a local of one function.

   It moves to src/lib/index-hover.ts verbatim — a delegated pointer
   pair that reads --share and writes four properties, needing nothing
   from the driver's scope. The lab keeps its copy so it still
   demonstrates hover when opened from disk; this deletes that copy and
   the JS_HEAD import supplies the shared one.

   Anchored on the signature and asserted, like every swap here: a
   rename in the lab that silently dropped this would ship a homepage
...
const curtainFinal = `/* GENERATED by scripts/port-home.mjs from public/lab/pressing-home.html.
 * Do not edit. Tune the lab, then run \`npm run port:home\`.
 *
 * The site's curtain, on its own. PressingTransition imports this, so
 * it stands on every route, including the ones that never load the
 * homepage's stylesheet. The tokens are the lab's, set on the curtain
 * itself, so a page that defines none of them draws the same curtain.
 */
.pt {
  --paper: ${PAPER};
  --ink: ${INK};
...
if (CHECK) {
  ...
  let curtainNow = "";
  try { curtainNow = readFileSync(CURTAIN_OUT, "utf8"); } catch { /* not written yet */ }
  if (curtainNow !== curtainFinal) stale.push(CURTAIN_OUT);
  if (stale.length) {
  ...
  writeFileSync(CURTAIN_OUT, curtainFinal);
  console.log(`ported ${LAB}`);
  console.log(`  ${CURTAIN_OUT}  ${curtainFinal.split("\n").length} lines`);
