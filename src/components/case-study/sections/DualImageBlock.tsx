  ...
  return (
    <div className={`${aspectClass} ${transparent ? "" : "rounded-[clamp(30px,5vw,60px)] bg-surface-alt"} overflow-hidden`}>
      {src ? (
        ...
        <img src={src} alt={alt} className={`w-full ${native && !aspect ? "h-auto" : "h-full object-cover"}`} style={blend ? { mixBlendMode: blend as React.CSSProperties["mixBlendMode"] } : undefined} />
      ) : (
