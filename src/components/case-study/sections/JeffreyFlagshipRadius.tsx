...
export function JeffreyFlagshipRadius() {
  const svgContent = useMemo(() => {
    ...
    VISUAL_ELEMENTS.forEach((ve, vi) => {
      ...
      const vx = px(cx + Math.cos(a) * R_VIS);
      const vy = px(cy + Math.sin(a) * R_VIS);
      els.push(
    ...
    channels.forEach((ch, ci) => {
      ...
      for (let r = 0; r < rayCount; r++) {
        ...
        const x1 = px(cx + Math.cos(rayAngle) * innerR);
        const y1 = px(cy + Math.sin(rayAngle) * innerR);
        const x2 = px(cx + Math.cos(rayAngle) * outerR);
        const y2 = px(cy + Math.sin(rayAngle) * outerR);
      ...
      const mx1 = px(cx + Math.cos(midAngle) * 60);
      const my1 = px(cy + Math.sin(midAngle) * 60);
      const mx2 = px(cx + Math.cos(midAngle) * 345);
      const my2 = px(cy + Math.sin(midAngle) * 345);
      els.push(
        <line
          key={`main-ray-${px(ci)}`}
    ...
    channels.forEach((ch, i) => {
      ...
      if (isBottom) {
        ...
        labelDefs.push(
          <path key={`flp-${i}`} id={`flagshipLabel-${i}`} d={`M ${px(x1)} ${px(y1)} A ${px(R_IL)} ${px(R_IL)} 0 0 0 ${px(x2)} ${px(y2)}`} fill="none" stroke="none" />
        );
      } else {
        const arcStart = midAngle - textArc / 2;
        const arcEnd = midAngle + textArc / 2;
        ...
        labelDefs.push(
          <path key={`flp-${i}`} id={`flagshipLabel-${i}`} d={`M ${px(x1)} ${px(y1)} A ${px(R_LABEL)} ${px(R_LABEL)} 0 0 1 ${px(x2)} ${px(y2)}`} fill="none" stroke="none" />
        );
      }
