...
- 2026-09-07 — **The board says where you are.** `scripts/lib/assemble-board.py` → `public/lab/board.html`: every standing rule carries the number of the column it opens (phone: `3 OF 79` at the column's head in the caption register, the peek's `4 OF 79 →` stacked in its fifth of a module) and the rule closing a column being read carries that column's depth as a 2px ink segment, the track running from the first-tile line to the bar (`dressRules`, redrawn on remount, page turn, column change and scroll). Desktop: the numbers replace the address in the cover line's meta slot (`1–2 of 62`, the pair on the glass; a study column is skipped); the address lives under Connect. Chosen as E of seven mockups in `public/lab/axis-variants.html`. Phone head band 30 → 48.
