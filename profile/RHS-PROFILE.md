...
- 2026-09-07 (later) — **The page is in the board's address.** `?open=` already stood the row back up (studies by folder, `house:`, `shelf:`); each id now carries `@n`, the module it is anchored after, so a restored column stands where it stood, and `?at=` names the page: the column at the near edge by its id, or the field's module by its 1-based number (`?at=43`). Written on every page turn (`writeRow` from `pageTo`, `history.replaceState`), applied after `standRow`. Verified: `?at=44&open=robert-rodriguez@43` restores Robert in the second place at 731 with `+18 →`.
- 2026-09-07 — **The board says where you are.** `scripts/lib/assemble-board.py` → `public/lab/board.html`: one count at the top right says what stands past the right edge, `+78 →` on the phone at column 1 (`+60 →` for the desktop's pair at 1–2), one fewer per page, gone at the last column; it replaces the address in the desktop cover line's meta slot (the address lives under Connect) and stands in the phone's head band over the peek (`#more`). The rule closing each column on the glass, field or study, carries that column's depth as a 2px ink segment, the track running from the first-tile line to the bar; a study column stands on the standing rule, so it draws the gauge on its own edge (`.ccol .rg`) (`dressRules`, redrawn on remount, page turn, column change and scroll). Chosen from seven mockups in `public/lab/axis-variants.html`: D for the count, E for the gauge, after per-column numbering (`3 OF 79`) was tried and dropped. Phone head band 30 → 48.
- 2026-09-06 — **The Ask answers from Sonnet 5, on a why bench.** `src/app/api/ask/route.ts` default moved from `claude-haiku-4-5` to `claude-sonnet-5` (`ASK_MODEL` still overrides, not set on Vercel). `npm run ask:bench -- --set why` sends twelve whys with the study's href so the route reads that study's prose (`Project.text`, first three picked, mined by `npm run facts`): both models 0 flagged sentences, but Haiku invented a why for A.R.C. and Sonnet answered from the study's sentence; 2,463ms against 3,080ms an answer. The cached prefix is 11,270 tokens (Sonnet floor 1,024; Haiku's was 4,096). The prompt carries the grey-half rule, ten specimens and the two-sentence rule; the route runs `src/lib/voice-tells.ts` over every answer with one retry. Divergence from Sally: Sally's chat stays on its own model; this is the site voice, chosen for fidelity to the study text over speed.
- 2026-09-05 — Board mobile pass in real Safari (iPhone 16 simulator): the phone's two feed columns scroll as one (the column under the last press drives, the other follows); a closed sheet loads nothing (36 images shelved until it opens, boot went 42→14 images); the rail's drawers filter on touch (pointerleave fires on a finger lifting, now mouse-only); a question in a column seats under the sticky head with instant scrolls and blurs the field; a width flip across 760 reloads into the right layout. `?debug=1` paints errors and tap targets into a strip on the glass, the only console a phone session has.
- 2026-09-04 (latest): **Each column of the board's field scrolls independently.** `public/lab/board.html`: the plane now holds one `.fcol` scroll container per module and keeps only X; the vertical belongs to whichever column the pointer is over, so the browser owns the wheel, touch pan, momentum and ends and the hand-rolled `setY`/`yMax`/`velY` are gone. A column windows itself against its own scrollTop (mounting whole works at desktop's 9 tiles per column and dies at a phone's 274) and remembers where it was left when paged away. Jeremy chose this over a density pass first, told that his columns are 30–34% picture and 66–70% air with gaps to 688px — so the air is still open. Same session: two columns plus a cut fifth of the third (`MOD_X = FIELD_W / 2.2`) so the row says it is a row, and a load BUILD that opens each column left to right with a clip-path edge on the plane (an earlier per-tile travel read as "groups" and was rejected — a column must have one clock).

- 2026-09-04: **The board works on a phone, as the same row.** `public/lab/board.html` at ≤760: the live site's two-up phone grid (VISIBLE stays 2, shares [1], ONE period, each column stacking on its own height), and a column is a PAGE in the row — it spans two modules, which is one screen, stands after the field's period and arrives from the right as the row glides to meet it. Swipe sideways between the field and the columns, up and down inside each; `PAGE = PHONE ? 2 : 1` and `pageTo` lands on whole pages and stops at the last column. A fixed sheet was tried first and Jeremy called it "just a fade" — the feel of the row was the thing lost. The chip head is sticky on a phone (WebKit measures sticky from the scroller's CONTENT box, so the scroller carries no top padding). Verified in mobile Safari on an iPhone 16 simulator (`xcrun simctl` + the simulator tool). Also: a study's pull home now lands on the BOARD, not `/` — `PressingHome` writes the board's own `pt.arrive` note and hard-navigates under the black; the board carries the app's arrival head script verbatim and lifts it. When the board moves to `/`, `HOME` in PressingHome is the one line. Gutters on the phone are 10, not the live 20: the period must equal the screen for a column page to align, and a dark column on an inset strip showed white bands.

- 2026-09-04 (later): **A case study ends at the homepage, not at a copy of it.** `PressingFooter` renders only the new `PressingHome` on `/case-studies/*`; contact, credits and `PressingRing` come off. The ring was the homepage's body rebuilt from the same sources under the study's URL, so a reader scrolling past the end landed on a front page that read as real until they clicked. A deliberate push past the end now plays the site's own curtain over `router.push("/")`. It is a PULL, not a tripwire, and that is the design: the flick that carries you to the bottom cannot count, because momentum keeps firing wheel events for ~1s after the fingers lift, so the pull arms only after the wheel goes quiet (`PULL` 260px, `QUIET` 260ms); a filling rule shows the pull; a button does the same act for anyone who never finds the gesture; touch needs no arming; `atEnd()` asks the Lenis-owned `<main>`, never the window. `lib/curtain.ts` gained `registerNavigator`/`curtainTo(label, sub, href)` so a caller borrows the sequence's own commit — the one that waits on the pathname rather than on `router.push` returning — instead of re-deriving what "arrived" means. This reverses a documented earlier decision (two prior versions of the return offered a door and a scroll-commit, both removed) and answers its objection rather than ignoring it. Divergence worth noting: contact and credits now live only on the homepage, which is where the board's Info/Connect columns are headed anyway.

- 2026-09-04: **`GET /api/book` added, and the board's footer became two rooms.** `/api/book` now answers GET with the same `availability()` that `/book` renders server-side (`{ok, minutes, days}`, `Cache-Control: no-store`) — the lab board (`public/lab/board.html`, the future homepage) is a static page that has to ask, and a week drawn from the rules alone lies the moment a slot is claimed. On the board there is no footer: Info and Connect are BLACK columns (paper = the work, black = the house), opened from the rail's `/ Info` `/ Connect` chips or by asking; Connect is /book's lede + the message field + the week as day rows with the open day's times as chips + Name/Email + Book (POST `/api/book`, the typed project riding as the note; 409 greys the time) + the ways in as plain links. Copy is mined at build (`npm run board` → `public/lab/board-copy.json` `house`) from PressingContact/PressingCredits/book page, loud if a block moves. Masthead reverses per SIDE over a black module and the burn pill hides. Divergence from Sally worth noting: the form is a conversation in a column, not a form; the calendar is the row anatomy, not a grid.

- 2026-08-30 (later): **Sitewide type shift to Avenir Next, and a real underline.** Helvetica Neue → Avenir Next across the shell (globals `.pressing`, Masthead, the three footer beats, the lab's `--sans`). Five weights were ALREADY self-hosted from the A.R.C./Ivy Park brand-system sections (500/600/700/800 + italics); 400 is not, and is the site's most-used weight — added as a `local()` face so it renders true on a Mac, flagged as the one thing that must be licensed before this is shippable off-Mac. Compensations, all measured against the face rather than guessed: display copy 500→600 and tracking to -0.05em (Jeremy's Figma value); the WORDMARK the other way, 700→600, because Avenir Bold carries more ink than Helvetica Bold; the ask field's driver-written `--ask-fw`/`--ask-ls` endpoints moved to 600/-0.05em so the field matches the statement it sits inside. **The drawn underline became a real one:** two stacked background gradients (kept because text-decoration cannot animate width) → `text-decoration` with `skip-ink`, `thickness: max(1px, 0.05em)`, `offset: 0.15em`. Cost: the left-to-right wipe, which no single element can do while also breaking around a descender. Hover now moves `text-decoration-color`. Applied to the ask field too — a placeholder takes the decoration, so the rest-state line survives, and `--ask-uw` retires. **Bug found by eye:** PressingContact was the only footer beat that never declared a font-family, so it had been rendering in Satoshi while every other beat was Helvetica; invisible until Avenir made it obvious. **Regression found and fixed same session:** the lead frame's hover push died because the port DELETES the lab's `armRows` and imports `src/lib/index-hover.ts` — the rewire went into the lab, which is thrown away. The port now diffs the custom properties each copy writes and refuses to run when they disagree.

- 2026-08-30: **The ledger became an instrument panel — the receipt shows the meal being made.** Jeremy's read after the seen layer landed: the retrieval numbers are more impressive than the model's sentence, since every AI product types prose and nobody shows the machinery. The "How it works" block now renders measured rows landing one at a time while the model thinks: Status, Read (30 case studies, 358 photographs, 2,192 catalogued terms), Caught (per-query: 19 projects, 97 photographs), Time (Nms on your machine, before any model), then Sources/Words when Haiku reports. Division of labor stated plainly: the index proves, the model speaks. Rules: every number measured this build (performance.now around think+frameMatch, frameMatch exposes its uncapped total, buildSeen counts its vocabulary once) — staged real quantities, never an invented "thinking" line; held plans (contact, owner questions) get NO rows because a hand-written answer read nothing; counts and machinery() moved out of the receipt prose into rows (one fact one home). Notes column now fades in with the deal instead of after the sentence, so the panel IS the show during the model hold. Same day, Jeremy promoted Read/Caught/Time out of the 12px rail into a stat band (`#ansStats`, both surfaces' markup + renderStats in the driver) directly under the say line at clamp(16px,1.7vw,23px) in the recessive grey — "so we don't bury the good stuff"; the rail keeps Status + Sources/Words/disclaimer.

- 2026-08-29: **The search reads the photographs at project level.** Jeremy searched "green" live and got "no match" while nineteen projects held green on camera (Jeffrey Spring 10 frames, Chalet 7, the kitchen's sage cabinetry 6). The vision index's 2,477 seen terms were reachable by frameMatch (picking frames) but never by factMatches (picking PROJECTS), so the deal and the model's hrefs ignored everything the camera knew. New `buildSeen()` aggregates images → href → word → frame-count once when the index lands; factMatches gains a `seen` facet (0.3/frame, capped at 2.4 so a photograph never outranks a mined fact; multi-word queries must land every word in one project's frames). Seen defers to written facets the way `keyword` does, and a `category` hit mutes seen-only additions entirely — "interiors" had grown 8→12 because A.R.C.'s app screenshots show rooms, and the rail's captured counts in rail-categories.ts would have drifted. Answer speaks in an observed register ("You can see green all through the photographs below"), receipt says "something the camera catches". Same fabrication ethos as the server's SEEN IN facts: visible, never promoted to intent.

- 2026-08-28: /book redesigned as "the spine, opened" (lab book-variants.html variant F): rotated month + stacked display numerals, the day under the pointer floods to ink and reveals its hours (the drawer language from the homepage rail). Copy rewritten by ear against VOICE.md. Server renders the first open day already open, so the page shows real times without script.
