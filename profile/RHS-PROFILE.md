...
- 2026-09-07 — **The board says where you are.** `scripts/lib/assemble-board.py` → `public/lab/board.html`: one count at the top right says what stands past the right edge, `+78 →` on the phone at column 1 (`+60 →` for the desktop's pair at 1–2), one fewer per page, gone at the last column; it replaces the address in the desktop cover line's meta slot (the address lives under Connect) and stands in the phone's head band over the peek (`#more`). The rule closing each column on the glass carries that column's depth as a 2px ink segment, the track running from the first-tile line to the bar (`dressRules`, redrawn on remount, page turn, column change and scroll). Chosen from seven mockups in `public/lab/axis-variants.html`: D for the count, E for the gauge, after per-column numbering (`3 OF 79`) was tried and dropped. Phone head band 30 → 48.
